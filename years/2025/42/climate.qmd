...
write_csv(
  ...
  here("years", "2025", "42", "data", "historic_station_met.csv")
)

write_csv(
  station_meta,
  here("years", "2025", "42", "data", "station_meta.csv")
)
```
...
```{r temp_change_calc, include=FALSE}
...
# Select years in two periods and label
period_df <- station_met_country |>
  filter(year %in% c(early_start:early_end, recent_start:recent_end)) |>
  mutate(period = case_when(
    year %in% early_start:early_end ~ paste0(early_start, "_", early_end),
    year %in% recent_start:recent_end ~ paste0(recent_start, "_", recent_end)
  ))

# Average monthly values into yearly values (one value per station per year)
...
# Calculate mean for each station across the 10 year period
...
# Calculate mean for each country across 10 year period
...
# Pivot wide to calculate temperature change
country_period_wide <- country_period_avg |>
  pivot_wider(
    names_from = period,
...
# Calculate temperature change and prepare for plotting
temp_change <- country_period_wide |>
  mutate(
  ...
  mutate(
    variable = recode(variable,
      "change_tmax" = "Max Temp",
      "change_tmin" = "Min Temp"
    ),
...
```{r temp_change_plot, fig.width=10, fig.height=6}
...
# Filter to last 35 years for clarity
...
```{r rainfall_plot, fig.width=10, fig.height=6}
